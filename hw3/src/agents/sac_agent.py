...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            self.log_alpha = torch.tensor(
                np.log(temperature), dtype=torch.float32, device=ptu.device, requires_grad=True
            )
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qs = self.target_critic(next_obs, next_action)  # (num_critics, batch_size)
            # ENDTODO
            ...
            target_values = reward[None] + self.discount * (1.0 - done[None]) * next_qs
            # ENDTODO
        ...
        # Predict Q-values
        q_values = self.critic(obs, action)  # (num_critics, batch_size)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # Use rsample so gradients can flow through when called outside no_grad context.
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # Use rsample so gradients flow through the sampled action to the actor parameters.
        action = action_distribution.rsample()
        ...
        q_values = self.critic(obs, action)  # (num_critic_networks, batch_size)
        assert q_values.shape == (self.num_critic_networks, batch_size), q_values.shape
        # ENDTODO
        ...
        # Maximize E[Q(s, a)] → minimize -E[Q(s, a)]
    ...
    def update_alpha(self, log_prob: torch.Tensor):
        ...
        # Gradient ascent on the dual: maximize alpha * (H - H_target)
        # Equivalently, minimize: -log_alpha * (log_prob + target_entropy)
