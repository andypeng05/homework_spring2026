    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                # Use online critic to SELECT action, target critic to EVALUATE it
                next_action = self.critic(next_obs).argmax(dim=-1)  # (batch_size,)
            else:
                next_action = next_qa_values.argmax(dim=-1)  # (batch_size,)
            ...
            target_values = reward + self.discount * (done.bool().logical_not()) * next_q_values
        ...
        q_values = qa_values.gather(1, action.long().unsqueeze(1)).squeeze(1)
